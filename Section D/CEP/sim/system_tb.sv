...
task off_chip_uart_receive;
  ...
  begin
    ...
    assert(off_chip_data_out == data)
      $display("PASSED! Expected : %d Actual %d",
               data,
               off_chip_data_out);
    else begin

      // FIX:
      // top.data_in does not exist anywhere inside z1top.
      // Removing it avoids the hierarchical lookup error.
      $error("FAILED! Expected : %d Actual %d",
             data,
             off_chip_data_out);
